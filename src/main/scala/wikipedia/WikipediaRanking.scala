...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setAppName("Wiki_guille").setMaster("local")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.filter(_.text.split(" ").contains(lang)).count().toInt
    //rdd.filter(_.text.contains()).count().toInt
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
            ...
            {for {
                art <- rdd
                lang <- langs
                if art.text.split(" ").contains(lang)
              } yield (lang, art)
            }.groupByKey()
...
//
    ...
    langsRanked2.take(10).foreach(println)
